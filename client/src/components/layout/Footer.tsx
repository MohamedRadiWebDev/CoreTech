      ...
      <Container className="py-14">
        <div className="grid gap-12 lg:grid-cols-[1.4fr,1fr,1fr]">
          <div className="space-y-5">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.28em] text-primary">Core Tech</p>
              <h2 className="mt-3 text-3xl font-semibold tracking-tight">{siteConfig?.company.tagline}</h2>
            </div>
            <p className="max-w-xl text-sm leading-7 text-slate-300">{t('footer.company_description')}</p>
          ...
          <div>
            ...
            <ul className="mt-5 space-y-3 text-sm text-slate-300">
              {navItems.map((item) => (
                <li key={item.href}>
                  <Link href={item.href}><a className="hover:text-white">{t(item.translationKey)}</a></Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="mt-5 space-y-3 text-sm text-slate-300">
              {services?.slice(0, 6).map((service) => (
                <li key={service.id}>
                  <Link href={`/services#${service.id}`}><a className="hover:text-white">{service.title}</a></Link>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="mt-12 flex flex-col gap-4 border-t border-white/10 pt-6 text-sm text-slate-400 sm:flex-row sm:items-center sm:justify-between">
          <p>© {year} Core Tech. {t('footer.all_rights_reserved')}</p>
